...
export default function RoomManagementView({ 
  ...
  const [quickGuestGst, setQuickGuestGst] = useState('');
  const [quickNumberOfPeople, setQuickNumberOfPeople] = useState(2);
  ...
  const handleSaveQuickCheckIn = (e: FormEvent) => {
    ...
    try {
      ...
      if (!isNaN(d1.getTime()) && !isNaN(d2.getTime())) {
        ...
        const extraBedsTotal = quickExtraBedsCount * (quickCheckInRoom.extraBedPrice || settings.extraBedPrice || 500);
    ...
    const updatedRoom: Room = {
      ...
      guestGst: quickGuestGst.trim() || undefined,
      numberOfPeople: quickNumberOfPeople,
                      ...
                      {room.status === 'booked' && (
                        <div>
                          ...
                          <div className="flex items-center gap-2 mt-1 text-[10px] text-white/50 font-mono">
                            <span>In: {formatDate(room.checkInDate) || 'N/A'}</span>
                            <span className="text-white/20">|</span>
                            <span>Out: {formatDate(room.checkOutDate) || 'N/A'}</span>
                          </div>
                          {room.extraBedsCount && room.extraBedsCount > 0 ? (
                            <span className="text-[9px] bg-indigo-500/15 border border-indigo-500/20 text-indigo-300 px-1.5 py-0.5 rounded font-semibold mt-1 inline-block">
                              +{room.extraBedsCount} Extra Bed
              ...
              <button
                ...
                onClick={() => {
                  ...
                  onAddRoom({
                    ...
                    extraBedPrice: settings.extraBedPrice || 500,
                    extraBedsCount: 0
                  });
                  setNewRoomId('');
              ...
              <div className="grid grid-cols-2 gap-3">
                <div>
                  ...
                  <input
                    type="number"
                    min="1"
                    value={quickNumberOfPeople}
                    onChange={(e) => setQuickNumberOfPeople(Number(e.target.value))}
                    className="w-full border border-white/10 bg-white/5 text-white rounded-xl p-2.5 text-xs focus:outline-none focus:border-indigo-500 transition-colors"
                    id="input-quick-number-of-people"
                  />
                </div>
                <div>
                  <label className="block text-[10px] font-bold text-white/40 uppercase tracking-wider mb-1">
                  ...
                  <input
                    ...
                    value={quickGuestGst}
                    onChange={(e) => setQuickGuestGst(e.target.value)}
                    className="w-full border border-white/10 bg-white/5 text-white placeholder-white/30 rounded-xl p-2.5 text-xs focus:outline-none focus:border-indigo-500 transition-colors"
                    id="input-quick-guest-gst"
                  />
                </div>
              </div>
