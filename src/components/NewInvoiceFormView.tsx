...
const DEFAULT_ITEMS_CATALOG = [
  { label: 'Room Service - Dining', defaultPrice: 850 },
  { label: 'Laundry Services', defaultPrice: 400 },
  { label: 'Airport Transfer', defaultPrice: 1200 },
...
export default function NewInvoiceFormView({ rooms, onSaveInvoice, onCancel, prefillData, settings }: NewInvoiceFormViewProps) {
  ...
  // Prefill occupant details on room shift
  useEffect(() => {
    if (selectedRoomId) {
  ...
  useEffect(() => {
    ...
    const roomStayDesc = `Room Stay - Room ${matchRoom.id} (${matchRoom.roomType})`;

    // Filter out any prior room stay or extra bed entries to keep catalog items unaffected
    const otherItems = lineItems.filter(
      item => !item.description.startsWith('Room Stay -') && !item.description.includes('Extra Bed')
    );
  ...
  const handleAddItemRow = () => {
    const defaultSelection = DEFAULT_ITEMS_CATALOG[0];
    ...
    setLineItems(lineItems.map((item) => {
      if (item.id === id) {
        ...
        if (field === 'description') {
          ...
          const matchCatalog = DEFAULT_ITEMS_CATALOG.find(cat => cat.label === value);
          if (matchCatalog) {
          ...
          <section className="glass-panel p-5 rounded-2xl shadow-lg lg:col-span-6 space-y-4">
            <div className="flex items-center justify-between pb-3 border-b border-white/10">
              <div className="flex items-center gap-2">
                ...
                <h3 className="text-base font-bold text-white font-display">Stay Details</h3>
              </div>
              {selectedRoomId && (
                  ...
                  <tr key={item.id} className="hover:bg-white/5 transition-colors group">
                    <td className="px-6 py-3">
                      ...
                      >
                        ...
                        <option value="Mini-Bar Beverages" className="bg-[#12121e]">Mini-Bar Beverages</option>
                      </select>
                    </td>
                    <td className="px-6 py-3">
                      <input 
