...
export default function RoomDrawer({ room, onClose, onSave, onDelete }: RoomDrawerProps) {
    ...
    if ((status === 'occupied' || status === 'booked') && checkInDate && checkOutDate && room) {
      try {
        ...
        if (!isNaN(d1.getTime()) && !isNaN(d2.getTime())) {
          ...
          const extraBedsTotal = extraBedsCount * (room.extraBedPrice || 500);
  ...
  }, [checkInDate, checkOutDate, extraBedsCount, status, room]);
